...
Different from `my-evaluate` (which evaluates models) and `my-status` (which checks progress).
This builds the data infrastructure the model trains on.

Output: `.note/DATA-PIPELINE.md` with pipeline code, statistics, and reproducibility notes
Commit: `data(phase-N): {description} - {N} samples {split} split`
</objective>

<execution_context>
@.github/my/workflows/data-prep.md
@.github/my/references/ml-data-engineering.md
</execution_context>
...
<process>
Execute the data-prep workflow from @.github/my/workflows/data-prep.md end-to-end.
