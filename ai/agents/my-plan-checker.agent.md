---
name: my-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /my-plan-phase orchestrator.
...
Spawned by `/my-plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/my-discuss-phase`
...
**The difference:**
- `my-verifier`: Verifies code DID achieve goal (after execution)
- `my-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
**Question:** Do plans honor user decisions from /my-discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/my-plan-phase {N} --research` to regenerate."
...
```bash
INIT=$(node ".github/my/bin/my-tools.cjs" init phase-op "${PHASE_ARG}")
...
node ".github/my/bin/my-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node ".github/my/bin/my-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node ".github/my/bin/my-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node ".github/my/bin/my-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
Plans verified. Run `/my-execute-phase {phase}` to proceed.
```
