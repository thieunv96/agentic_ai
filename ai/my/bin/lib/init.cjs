 ...
 * Workflows use this to prefix `.note/` paths correctly when the agent's CWD
...
function cmdInitExecutePhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'my-executor'),
    verifier_model: resolveModelInternal(cwd, 'my-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'my-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'my-planner'),
    checker_model: resolveModelInternal(cwd, 'my-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.ai', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Detect Firecrawl API key availability
  const firecrawlKeyFile = path.join(homedir, '.ai', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.ai', 'exa_api_key');
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.note', '.claude', '__pycache__', 'target', 'dist', 'build']);
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'my-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'my-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'my-roadmapper'),

    // Config
    commit_docs: config.commit_docs,

    // Existing state
    project_exists: pathExistsInternal(cwd, '.note/PROJECT.md'),
    has_codebase_map: pathExistsInternal(cwd, '.note/codebase'),
    planning_exists: pathExistsInternal(cwd, '.note'),

    // Brownfield detection
    has_existing_code: hasCode,
    has_package_file: hasPackageFile,
    is_brownfield: hasCode || hasPackageFile,
    needs_codebase_map: (hasCode || hasPackageFile) && !pathExistsInternal(cwd, '.note/codebase'),
...
function cmdInitQuick(cwd, description, raw) {
  ...
  const result = {
    ...
    // Paths
    quick_dir: '.note/quick',
    task_dir: slug ? `.note/quick/${quickId}-${slug}` : null,
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'my-codebase-mapper'),
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, '.note'),
    codebase_dir_exists: pathExistsInternal(cwd, '.note/codebase'),
  };
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /my-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /my-new-milestone first.');
  }
  ...
  try {
    const waitingPath = path.join(cwd, '.note', 'WAITING.json');
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/my-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/my-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/my-discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'my-workspaces');
...
function cmdInitListWorkspaces(cwd, raw) {
  ...
  if (fs.existsSync(defaultBase)) {
    ...
    for (const entry of entries) {
      ...
      hasProject = fs.existsSync(path.join(wsPath, '.note', 'PROJECT.md'));
