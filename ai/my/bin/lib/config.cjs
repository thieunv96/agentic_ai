 ...
 *   2. User-level defaults from ~/.ai/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /my-new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.ai/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.ai', 'defaults.json');
  let userDefaults = {};
  try {
...
/**
 * Command: create a fully-materialized .note/config.json for a new project.
 *
 * Accepts user-chosen settings as a JSON string (the keys the user explicitly
 * configured during /my-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.ai/defaults.json.
 *
...
function cmdConfigNewProject(cwd, choicesJson, raw) {
  ...
  try {
    ...
    error('Failed to create .note directory: ' + err.message);
  }
  ...
  try {
    ...
    output({ created: true, path: '.note/config.json' }, raw, 'created');
  } catch (err) {
...
function ensureConfigFile(cwd) {
  ...
  try {
    fs.writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf-8');
    return { created: true, path: '.note/config.json' };
  } catch (err) {
