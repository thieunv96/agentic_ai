...
/**
 * Walk up from `startDir` to find the project root that owns `.note/`.
 *
 * In multi-repo workspaces, the agent may open inside a sub-repo (e.g. `backend/`)
 * instead of the project root. This function prevents `.note/` from being
 ...
 * 1. Parent has `.note/config.json` with `sub_repos` listing this directory
 * 2. Parent has `.note/config.json` with `multiRepo: true` (legacy format)
 * 3. Parent has `.note/` and current dir has its own `.git` (heuristic)
 *
 * Returns `startDir` unchanged when no ancestor `.note/` is found (first-run
...
function findProjectRoot(startDir) {
  ...
  while (dir !== root) {
    ...
    const parentPlanning = path.join(parent, '.note');
    if (fs.existsSync(parentPlanning) && fs.statSync(parentPlanning).isDirectory()) {
      ...
      try {
        ...
        const subRepos = config.sub_repos || config.note?.sub_repos || [];
      ...
      // Heuristic: parent has .note/ and we're inside a git repo
...
/**
 * Remove stale ai-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'my-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'my-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `my-${Date.now()}.json`);
...
function loadConfig(cwd) {
  ...
  try {
    ...
    if (parsed.multiRepo === true && !parsed.sub_repos && !parsed.note?.sub_repos) {
      const detected = detectSubRepos(cwd);
      if (detected.length > 0) {
        parsed.sub_repos = detected;
        if (!parsed.note) parsed.note = {};
        parsed.note.commit_docs = false;
    ...
    const currentSubRepos = parsed.sub_repos || parsed.note?.sub_repos || [];
    ...
    return {
      ...
      commit_docs: (() => {
        ...
        // Auto-detection: when no explicit value and .note/ is gitignored,
        // default to false instead of true
        if (isGitIgnored(cwd, '.note/')) return false;
...
function isGitIgnored(cwd, targetPath) {
  try {
    ...
    // .gitignore explicitly lists them — a common source of confusion when .note/
 ...
 * Applied at write points so AI-generated .note/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 * In a linked worktree, .note/ lives in the main worktree, not in the linked one.
 * Returns the main worktree path, or cwd if not in a worktree.
 */
function resolveWorktreeRoot(cwd) {
  // If the current directory already has its own .note/, respect it.
  // This handles linked worktrees with independent planning state (e.g., Conductor workspaces).
  if (fs.existsSync(path.join(cwd, '.note'))) {
    return cwd;
  }
...
/**
 * Acquire a file-based lock for .note/ writes.
...
/**
 * Get the active workstream name from .note/active-workstream file.
...
function findPhaseInternal(cwd, phase) {
  ...
  const milestonesDir = path.join(cwd, '.note', 'milestones');
  ...
  try {
    ...
    for (const archiveName of archiveDirs) {
      ...
      const relBase = '.note/milestones/' + archiveName;
    ...
    for (const archiveName of phaseDirs) {
      ...
      for (const dir of dirs) {
        results.push({
          name: dir,
          milestone: version,
          basePath: path.join('.note', 'milestones', archiveName),
