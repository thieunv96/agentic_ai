...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const milestonesPath = path.join(cwd, '.note', 'MILESTONES.md');
  const archiveDir = path.join(cwd, '.note', 'milestones');
  ...
  if (fs.existsSync(reqPath)) {
    ...
    const archiveHeader = `# Requirements Archive: ${version} ${milestoneName}\n\n**Archived:** ${today}\n**Status:** SHIPPED\n\nFor current requirements, see \`.note/REQUIREMENTS.md\`.\n\n---\n\n`;
    fs.writeFileSync(path.join(archiveDir, `${version}-REQUIREMENTS.md`), archiveHeader + reqContent, 'utf-8');
  }

  // Archive audit file if exists
  const auditFile = path.join(cwd, '.note', `${version}-MILESTONE-AUDIT.md`);
