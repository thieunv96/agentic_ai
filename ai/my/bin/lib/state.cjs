...
function cmdStateLoad(cwd, raw) {
  ...
  const planDir = planningPaths(cwd).note;
...
/**
 * Write a WAITING.json signal file when AI hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .note/WAITING.json (or .ai/WAITING.json if .ai exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const aiDir = fs.existsSync(path.join(cwd, '.ai')) ? path.join(cwd, '.ai') : planningDir(cwd);
  const waitingPath = path.join(aiDir, 'WAITING.json');
