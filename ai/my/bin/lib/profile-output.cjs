...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /my-new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  'Before using Edit, Write, or other file-changing tools, start work through a AI command so planning artifacts and execution context stay in sync.',
  '',
  'Use these entry points:',
  '- `/my-quick` for small fixes, doc updates, and ad-hoc tasks',
  '- `/my-debug` for investigation and bug fixing',
  '- `/my-execute-phase` for planned phase work',
  '',
  'Do not make direct repo edits outside a AI workflow unless the user explicitly asks to bypass it.',
].join('\n');
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/my-profile-user` to generate your developer profile.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- AI:${sectionName}-start`;
  const endMarker = `<!-- AI:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- AI:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- AI:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateProjectSection(cwd) {
  const projectPath = path.join(cwd, '.note', 'PROJECT.md');
...
function generateStackSection(cwd) {
  const codebasePath = path.join(cwd, '.note', 'codebase', 'STACK.md');
  const researchPath = path.join(cwd, '.note', 'research', 'STACK.md');
...
function generateConventionsSection(cwd) {
  const conventionsPath = path.join(cwd, '.note', 'codebase', 'CONVENTIONS.md');
...
function generateArchitectureSection(cwd) {
  const architecturePath = path.join(cwd, '.note', 'codebase', 'ARCHITECTURE.md');
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'ai', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/my-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'ai', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/my-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by AI from ${dataSource}. Run \`/my-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- AI:profile-start -->';
    const endMarker = '<!-- AI:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## AI Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- AI:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- AI:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- AI:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- AI:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /my-profile-user to unlock Developer Profile.';
