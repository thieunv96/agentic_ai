 ...
 * REQUIREMENTS.md, and phases/ into .note/workstreams/{name}/ directories.
 *
 * When no workstreams/ directory exists, AI operates in "flat mode" with
 * everything at .note/ — backward compatible with pre-workstream installs.
 */
...
/**
 * Migrate flat .note/ layout to workstream mode.
 * Moves per-workstream files (ROADMAP.md, STATE.md, REQUIREMENTS.md, phases/)
 * into .note/workstreams/{name}/. Shared files (PROJECT.md, config.json,
...
function migrateToWorkstreams(cwd, workstreamName) {
  ...
  if (fs.existsSync(path.join(baseDir, 'workstreams'))) {
    throw new Error('Already in workstream mode — .note/workstreams/ exists');
  }
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error('.note/ directory not found — run /my-new-project first');
  }
