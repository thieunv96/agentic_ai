...
function cmdCommit(cwd, message, files, raw, amend, noVerify) {
  ...
  // Check if .note is gitignored
  if (isGitIgnored(cwd, '.note')) {
  ...
  // Stage files
  const filesToStage = files && files.length > 0 ? files : ['.note/'];
...
function cmdCommitToSubrepo(cwd, message, files, raw) {
  ...
  if (!subRepos || subRepos.length === 0) {
    error('no sub_repos configured in .note/config.json');
  }
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /my-discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
