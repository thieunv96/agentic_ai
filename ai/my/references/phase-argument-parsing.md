...
```bash
PHASE_INFO=$(node ".github/my/bin/my-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".github/my/bin/my-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".github/my/bin/my-tools.cjs" find-phase "${PHASE}" --raw)
```
