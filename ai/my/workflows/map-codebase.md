<purpose>
Orchestrate parallel codebase mapper agents to analyze codebase and produce structured documents in .note/codebase/

Each agent has fresh context, explores a specific focus area, and **writes documents directly**. The orchestrator only receives confirmation + line counts, then writes a summary.

Output: .note/codebase/ folder with 7 structured documents about the codebase state.
</purpose>
...
```bash
INIT=$(node ".github/my/bin/my-tools.cjs" init map-codebase)
...
Check if .note/codebase/ already exists using `has_maps` from init context.
...
```
.note/codebase/ already exists with these documents:
[List files found]
...
If "Refresh": Delete .note/codebase/, continue to create_structure
...
Create .note/codebase/ directory:

```bash
mkdir -p .note/codebase
```
...
Spawn 4 parallel my-codebase-mapper agents.

Use Task tool with `subagent_type="my-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `my-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="my-codebase-mapper",
  model="{mapper_model}",
...
Write these documents to .note/codebase/:
...
Write this document to .note/codebase/:
...
**Focus:** {focus}
**Documents written:**
- `.note/codebase/{DOC1}.md` ({N} lines)
- `.note/codebase/{DOC2}.md` ({N} lines)
...
- Write `.note/codebase/STACK.md` — Languages, runtime, frameworks, dependencies, configuration
- Write `.note/codebase/INTEGRATIONS.md` — External APIs, databases, auth providers, webhooks

**Pass 2: Architecture Focus**
- Explore directory structure, entry points, module boundaries, data flow
- Write `.note/codebase/ARCHITECTURE.md` — Pattern, layers, data flow, abstractions, entry points
- Write `.note/codebase/STRUCTURE.md` — Directory layout, key locations, naming conventions

**Pass 3: Quality Focus**
- Explore code style, error handling patterns, test files, CI config
- Write `.note/codebase/CONVENTIONS.md` — Code style, naming, patterns, error handling
- Write `.note/codebase/TESTING.md` — Framework, structure, mocking, coverage

**Pass 4: Concerns Focus**
- Explore TODOs, known issues, fragile areas, security patterns
- Write `.note/codebase/CONCERNS.md` — Tech debt, bugs, security, performance, fragile areas

Use the same document templates as the `my-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
grep -E '(sk-[a-zA-Z0-9]{20,}|sk_live_[a-zA-Z0-9]+|sk_test_[a-zA-Z0-9]+|ghp_[a-zA-Z0-9]{36}|gho_[a-zA-Z0-9]{36}|glpat-[a-zA-Z0-9_-]+|AKIA[A-Z0-9]{16}|xox[baprs]-[a-zA-Z0-9-]+|-----BEGIN.*PRIVATE KEY|eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.)' .note/codebase/*.md 2>/dev/null && SECRETS_FOUND=true || SECRETS_FOUND=false
```
...
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: map existing codebase" --files .note/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/my-map-codebase`
- Review specific file: `cat .note/codebase/STACK.md`
...
<success_criteria>
- .note/codebase/ directory created
- If Task tool available: 4 parallel my-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in AI style
</success_criteria>
