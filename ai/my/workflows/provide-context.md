> **DEPRECATED** — Context gathering is now integrated into `/my-discuss N` (Step 0).
> When you run `/my-discuss N`, it will ask about papers, code references, and ideas before proceeding to decisions.
> This workflow is kept for reference only and should not be invoked directly.
...
Check whether `.note/ROADMAP.md` exists:
...
- Create `.note/ROADMAP.md` from the knowledge gathered:
```markdown
...
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: update context for [version-name]" --files .note/KNOWLEDGE.md .note/STATE.md
```

(Include `.note/ROADMAP.md` in commit only if ROADMAP.md was newly created in fallback mode.)
