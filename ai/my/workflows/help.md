...
- Creates `.note/debug/{slug}.md` tracking investigation
- Scientific method: evidence → hypothesis → test
- Survives `/clear` — run with no args to resume
- Archives to `.note/debug/resolved/` when solved
...
- Creates `.note/codebase/` with 7 focused documents:
...
`version` also creates: `.note/VERSION-REPORT.md` (permanent)
