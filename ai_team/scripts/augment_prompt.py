...
Usage:
    python tools/augment_prompt.py "your raw prompt here"
    python tools/augment_prompt.py "your raw prompt here" --task coding
"""
...
def build_tool_rules(task: str) -> str:
    if task == "coding":
        return (
            ...
            "Validate the final change with the narrowest useful checks before broadening scope."
        )
    if task == "research":
...
def build_output_contract(task: str) -> str:
    if task == "coding":
        return (
            "Return the result in a practical execution format: "
            "concise summary, concrete changes or code, validation notes, and any remaining risks."
        )
    if task == "research":
        return (
            "Return a structured synthesis with key findings, supporting evidence, "
            "uncertainty where relevant, and a concise bottom line."
        )
    if task == "writing":
        return (
    ...
    if task == "review":
        return (
            "Return findings grouped by severity or importance, explain why each matters, "
            "and suggest the smallest credible next step."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Optional explicit task type (auto-detected if omitted).",
    )
