...
```bash
INIT=$(node ".github/ai/bin/ai-tools.cjs" init map-codebase)
...
Spawn 4 parallel ai-codebase-mapper agents.

Use Task tool with `subagent_type="ai-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `ai-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="ai-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `ai-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node ".github/ai/bin/ai-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/ai-map-codebase`
...
- If Task tool available: 4 parallel ai-codebase-mapper agents spawned with run_in_background=true
