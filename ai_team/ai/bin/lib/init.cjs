...
function cmdInitExecutePhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'ai-executor'),
    verifier_model: resolveModelInternal(cwd, 'ai-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'ai-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'ai-planner'),
    checker_model: resolveModelInternal(cwd, 'ai-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'ai-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'ai-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'ai-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'ai-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /ai-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /ai-new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/ai-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/ai-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/ai-discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'ai-workspaces');
