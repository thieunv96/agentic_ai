 ...
 * @param {string} prefix - filename prefix to match (e.g., 'ai-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'ai-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `ai-${Date.now()}.json`);
 ...
 * Applied at write points so AI-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
