...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /ai-new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  ...
  '- `/ai-quick` for small fixes, doc updates, and ad-hoc tasks',
  '- `/ai-debug` for investigation and bug fixing',
  '- `/ai-execute-phase` for planned phase work',
  '',
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/ai-profile-user` to generate your developer profile.',
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/ai-profile-user --refresh` with session data to populate.';
  ...
  const result = {
    ...
    command_name: '/ai-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by AI from ${dataSource}. Run \`/ai-profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /ai-profile-user to unlock Developer Profile.';
