...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  reapStaleTempFiles('ai-pipeline-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  reapStaleTempFiles('ai-profile-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-profile-'));
