...
- `ai-researcher` — Reads decisions to focus research (e.g., "ViT backbone" → research ViT fine-tuning patterns for this task)
- `ai-planner` — Reads decisions to create specific tasks (e.g., "LoRA fine-tuning" → task includes LoRA adapter implementation)
- `ai-data-analyst` — Reads dataset decision to scope analysis
...
- `ai-phase-researcher` uses decisions to focus investigation AND reads canonical_refs to know WHAT docs to study
- `ai-planner` uses decisions + research to create executable tasks AND reads canonical_refs to verify alignment
