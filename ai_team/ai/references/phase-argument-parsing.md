...
```bash
PHASE_INFO=$(node ".github/ai/bin/ai-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".github/ai/bin/ai-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".github/ai/bin/ai-tools.cjs" find-phase "${PHASE}" --raw)
```
