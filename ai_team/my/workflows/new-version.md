...
Detect whether the repo already has substantial code (non-.planning files, non-README files).
...
> "Let's configure how the framework will run for this project. I'll ask a few setup questions — you can change these later in `.planning/config.json`."
...
Save to `.planning/config.json`. Commit config:
```bash
node ".github/my/bin/my-tools.cjs" commit "chore: initialize config for [version-name]" --files .planning/config.json
```
...
Commit:
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: project definition for [version-name]" --files .planning/PROJECT.md
```
...
Synthesize into RESEARCH-SUMMARY.md at .planning/RESEARCH-SUMMARY.md
```

Then spawn my-research-synthesizer to create `.planning/RESEARCH-SUMMARY.md`. Show key findings to user:
- Recommended stack
...
Create `.planning/REQUIREMENTS.md` using the requirements template. Requirements must be:
...
Commit:
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: requirements v1 for [version-name]" --files .planning/REQUIREMENTS.md .planning/PROJECT.md
```
...
Generate:
- `.planning/ROADMAP.md` with phases table
...
Commit:
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: roadmap for [version-name] ([N] phases, [M] requirements)" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md .planning/STATE.md
```
...
- `.planning/REQUIREMENTS.md` — [M] requirements with IDs
- `.planning/ROADMAP.md` — [N]-phase roadmap
...
```bash
node ".github/my/bin/my-tools.cjs" commit "chore: start [version-name] — update PROJECT.md + STATE.md" --files .planning/PROJECT.md .planning/STATE.md
```
