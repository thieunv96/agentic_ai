...
```bash
node ".github/my/bin/my-tools.cjs" commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
node ".github/my/bin/my-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node ".github/my/bin/my-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
