---
name: ai-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /ai-debug orchestrator.
...
- `/ai-debug` command (interactive debugging)
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /ai-debug to resume" if context filling up.
</step>
...
```bash
node ".github/ai/bin/ai-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
Resolved debug sessions. Used by `ai-debugger` to surface known-pattern hypotheses at the start of new investigations.
...
```bash
node ".github/ai/bin/ai-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
