---
name: ai-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /ai-plan-phase orchestrator.
...
Spawned by:
- `/ai-plan-phase` orchestrator (standard phase planning)
- `/ai-plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/ai-plan-phase` in revision mode (updating plans based on checker feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/ai-discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/ai-research-phase` before plan-phase.
...
<execution_context>
@.github/ai/workflows/execute-plan.md
@.github/ai/templates/summary.md
</execution_context>
...
```bash
node ".github/ai/bin/ai-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node ".github/ai/bin/ai-tools.cjs" history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /ai-discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /ai-research-phase
...
```bash
VALID=$(node ".github/ai/bin/ai-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ".github/ai/bin/ai-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node ".github/ai/bin/ai-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/ai-execute-phase {phase}`
...
Execute: `/ai-execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/ai-execute-phase {X}` next
