...
```bash
INIT=$(node ".github/asilla/bin/asilla-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse: `commit_docs`, `project_exists`, `has_git`, `project_path`.

If `project_exists` is true: Error — version already initialized. Use asilla-status.
...
## Current Position
Phase: Not started
Status: Ready for context — run /asilla-provide-context
...
```bash
[ -d .git ] || git init
node ".github/asilla/bin/asilla-tools.cjs" commit "docs: initialize [version-name] ([task-type])" --files .planning/
```
