...
```bash
INIT=$(node ".github/asilla/bin/asilla-tools.cjs" init plan-phase "0")
...
## 4. Quick Research (via asilla-ml-researcher)

Spawn asilla-ml-researcher with:
```
<files_to_read>
.planning/PROJECT.md
...
Update STATE.md:
- Status: "Ready to discuss — run /asilla-discuss 1"
- Add pointer to KNOWLEDGE.md and ROADMAP.md

## 7. Commit

```bash
node ".github/asilla/bin/asilla-tools.cjs" commit "docs: setup context for [version-name]" --files .planning/KNOWLEDGE.md .planning/ROADMAP.md .planning/STATE.md
```
...
---
**Also available:**
- `/asilla-status` — review full roadmap
```
