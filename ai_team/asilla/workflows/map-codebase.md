...
```bash
INIT=$(node ".github/asilla/bin/asilla-tools.cjs" init map-codebase)
...
Spawn 4 parallel asilla-codebase-mapper agents.

Use Task tool with `subagent_type="asilla-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `asilla-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="asilla-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `asilla-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node ".github/asilla/bin/asilla-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/asilla-map-codebase`
...
- If Task tool available: 4 parallel asilla-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in Asilla style
</success_criteria>
