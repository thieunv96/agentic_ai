...
```bash
INIT=$(node ".github/asilla/bin/asilla-tools.cjs" init report "0")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Read: STATE.md, PROJECT.md, ROADMAP.md

Create: `mkdir -p .planning/reports`
...
Produce `.planning/reports/session-{date}.md`:
...
Produce `.planning/reports/experiments-{date}.md`:
...
Produce `.planning/VERSION-REPORT.md` (permanent) + `.planning/reports/version-{version}-{date}.md`:
...
**Type:** {type}
**File:** `.planning/reports/{filename}`
