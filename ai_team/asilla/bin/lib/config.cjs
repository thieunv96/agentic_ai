 ...
 *   2. User-level defaults from ~/.asilla/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /asilla-new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.asilla/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.asilla', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * configured during /asilla-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.asilla/defaults.json.
 *
