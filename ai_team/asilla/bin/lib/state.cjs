...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { asilla_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when Asilla hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .asilla/WAITING.json if .asilla exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const asillaDir = fs.existsSync(path.join(cwd, '.asilla')) ? path.join(cwd, '.asilla') : planningDir(cwd);
  const waitingPath = path.join(asillaDir, 'WAITING.json');
  ...
  try {
    fs.mkdirSync(asillaDir, { recursive: true });
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.asilla', 'WAITING.json'),
