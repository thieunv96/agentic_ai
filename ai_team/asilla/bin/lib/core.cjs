...
/**
 * Remove stale asilla-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'asilla-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'asilla-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `asilla-${Date.now()}.json`);
 ...
 * Applied at write points so Asilla-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 * When a workstream is active (via explicit ws arg or Asilla_WORKSTREAM env var),
 * returns `.planning/workstreams/{ws}/`. Otherwise returns `.planning/`.
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks Asilla_WORKSTREAM env var
 */
function planningDir(cwd, ws) {
  if (ws === undefined) ws = process.env.Asilla_WORKSTREAM || null;
