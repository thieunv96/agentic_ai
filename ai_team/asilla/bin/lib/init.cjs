...
function cmdInitExecutePhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'asilla-executor'),
    verifier_model: resolveModelInternal(cwd, 'asilla-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'asilla-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'asilla-planner'),
    checker_model: resolveModelInternal(cwd, 'asilla-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.asilla', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Detect Firecrawl API key availability
  const firecrawlKeyFile = path.join(homedir, '.asilla', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.asilla', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'asilla-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'asilla-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'asilla-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'asilla-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /asilla-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /asilla-new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/asilla-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/asilla-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/asilla-discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'asilla-workspaces');
