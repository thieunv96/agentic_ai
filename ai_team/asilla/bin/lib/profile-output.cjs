...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /asilla-new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  'Before using Edit, Write, or other file-changing tools, start work through a Asilla command so planning artifacts and execution context stay in sync.',
  '',
  'Use these entry points:',
  '- `/asilla-quick` for small fixes, doc updates, and ad-hoc tasks',
  '- `/asilla-debug` for investigation and bug fixing',
  '- `/asilla-execute-phase` for planned phase work',
  '',
  'Do not make direct repo edits outside a Asilla workflow unless the user explicitly asks to bypass it.',
].join('\n');

const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  '<!-- Asilla:profile-start -->',
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/asilla-profile-user` to generate your developer profile.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- Asilla:${sectionName}-start`;
  const endMarker = `<!-- Asilla:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- Asilla:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- Asilla:${sectionName}-end -->`,
  ].join('\n');
}
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'asilla', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/asilla-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'asilla', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/asilla-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by Asilla from ${dataSource}. Run \`/asilla-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- Asilla:profile-start -->';
    const endMarker = '<!-- Asilla:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## Asilla Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- Asilla:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- Asilla:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- Asilla:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- Asilla:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /asilla-profile-user to unlock Developer Profile.';
