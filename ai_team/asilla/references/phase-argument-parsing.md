...
```bash
PHASE_INFO=$(node ".github/asilla/bin/asilla-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".github/asilla/bin/asilla-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".github/asilla/bin/asilla-tools.cjs" find-phase "${PHASE}" --raw)
```
