...
/ml-discuss [N]           ← surface real painpoints → lock requirements
       ↓
/ml-plan [N]              ← break into waves and tasks → PLAN.md
       ↓
/ml-implement [N]         ← execute autonomously → COMMIT
       ↓
/ml-test [N]              ← validate → GO/NO-GO → auto-proceeds
       ↓
/ml-doc [N]               ← generate user docs → COMMIT → auto-proceeds
       ↓
/ml-report                ← consolidate results → next step recommendations
```

`/ml-implement`, `/ml-test`, and `/ml-doc` run autonomously without confirmation. The pipeline only pauses when something needs your judgment: targets not met, a blocker found, or a gap in the implementation.
...
3. Optionally spawns **ml-researcher** to validate assumptions before locking requirements
...
Creates a detailed, wave-structured implementation plan from the locked requirements.
...
3. Builds a wave-structured plan with tasks, outputs, and acceptance criteria
4. Spawns **ml-rubber-duck** agent to validate task clarity before showing you
5. Presents the plan for adjustment — no cap on rounds
...
Executes the plan wave by wave, autonomously. No confirmation between tasks.
...
├── agents/
│   ├── ml-researcher.agent.md      ← research papers/approaches
...
|-------|-----------|-------------|
| `ml-researcher` | `ml-discuss` Step 5 | Web research, updates KNOWLEDGE.md, returns key findings |
| `ml-rubber-duck` | `ml-planning` Step 6 | Validates plan clarity, dependencies, failure modes — read-only |
| `ml-executor` | `ml-implement` per wave | Executes tasks, applies R1-R4 deviation rules, returns wave report |
| `ml-evaluator` | `ml-test` | Runs all checks, writes EVALUATION.md, returns decision |
...
# → optionally researches multi-scale feature fusion approaches
...
/ml-report
# → session report: what was done, metrics, recommended Phase 3 (latency optimization)
```
